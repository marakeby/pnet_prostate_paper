...
from utils.plots import generate_plots, plot_roc, plot_confusion_matrix
...
class TrainValidatePipeline:
    ...
    def run(self):
        ...
        # cnv_split =True
        # get data
        # x_train, x_test, y_train, y_test, info_train, info_test, cols = data.get_train_test()
        # x_train, x_test, y_train, y_test, info_train, info_test, cols = data.get_train_test()
        ...
        # x_test, _ = self.preprocess(x_test, x_test)
        ...
        # if self.features_params!={} and self.features_params['type'] =='scale_features':
        #     scaler = FeatureScaler(**self.features_params['params'])
        ...
        if type(self.model_params) ==list:
            for m in self.model_params:
                ...
                print m
                # if 'fitting_params' in m['params']:
                #     if 'x_to_list' in m['params']['fitting_params']:
                #         if m['params']['fitting_params']['x_to_list']:
                ...
                def predict(x_test, y_test, info_test, model_name, test_set_name):
                    ...
                    # y_pred_test = (y_pred_test1 + y_pred_test2) >1
                    ...
                    # plot_roc(y_test, y_pred_test_scores, self.directory, label=model_name)
                ...
                # logging.info('saving results')
                # self.save_score(test_score, model_name)
                # self.save_prediction(info_test, y_pred_test, y_test, model_name)
                #
                # plot_roc(y_test, y_pred_test_scores, self.directory, label=model_name)
                #
                # print 'getting layer outputs'
                # x_train_layer_outputs = model.get_layer_outputs(x_train)
                # x_test_layer_outputs = model.get_layer_outputs(x_test)
                # for xx in x_train_layer_outputs:
                #     print xx[0].shape


                # self.save_layer_outputs(x_test_layer_outputs, y_pred_test, marker='X')

                # if self.save_train:
                #     y_pred_train1, y_pred_train_scores1= self.predict(model1, x_train, y_train)
                #     y_pred_train2, y_pred_train_scores2= self.predict(model2, x_train, y_train)
                #     , score, train_cnf_matrix
                #     self.save_prediction(pd.DataFrame(index= info_train), y_pred_train, y_train, model_name,  training=True)

                    # self.save_layer_outputs(x_train_layer_outputs, y_train, y_pred_train, x_test_layer_outputs, y_pred_test)

                # if hasattr(model, 'coef_') and not model.coef_ is None:
                #     logging.info("dimensionality: %d" % model.coef_.shape[1])
                #     logging.info("density: %f" % density(model.coef_))
                #     model_list.append((model, model_name))


        # plt.savefig(join(self.directory, 'auc_curves'))
