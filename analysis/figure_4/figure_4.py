  ...
  for i, gene in enumerate(interesting):
      if gene in df.index:
          ...
          ax2.annotate(gene, (x, y), xycoords='data',fontsize=fontsize,
              ...
              xytext=xy_dict[gene], textcoords='offset points', ha='center',arrowprops=dict(arrowstyle="->",linewidth=0.3) )
...
def plot_stacked_hist_cnv( gene_name, ax):
    ...
    D_id_color = {'High amplification': 'maroon','Amplification': 'lightcoral',  'Neutral': 'gainsboro', 'Deletion': 'skyblue', 'Deep deletion': 'steelblue'}
...
def plot_crispr(ax):
    ...
    filename = join(PROSTATE_DATA_PATH, 'functional/revision/KD MDM4 proliferation raw data of 3 experiments in 4 cell lines in triplicates_procssed.xls')
    ...
    def grouped_barplot(df, cat, subcat, val, err):
        ...
        for i, gr in enumerate(subx):
            ...
            ax.bar(x + offsets[i], dfg[val].values, width=width,
                    label="{}".format(gr), yerr=dfg[err].values, capsize=1.5, color=colors[gr], alpha=0.7, error_kw={'elinewidth':0.5,'capthick':0.5})
        #     plt.xlabel(cat)
...
def plot_crispr_sem(ax):
    ...
    def grouped_barplot( df_in, cat_col, subcat_col, val_col):
        ...
        for i, gr in enumerate(subcats):
            ...
            ax.bar(x + offsets[i], means, width=width, yerr=stds, label="{}".format(gr), capsize=1.5, color=colors_dict[gr],
