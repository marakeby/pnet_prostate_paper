...
def plot_high_genes_violinplot(df_in,ax, y, name, saving_dir):
    ...
    sns.violinplot(y="variable", x="value", hue="group", data=df2, split=True, bw=.6, inner=None, palette=palette, linewidth=0., ax=ax)
...
def plot_activation(ax, column='coef_combined', layer=1,):
    ...
    current_palette = sns.color_palette()
    palette = dict(Primary=current_palette[0], Metastatic=current_palette[1])
    sns.violinplot(y="variable", x="value", hue="group", data=df2, split=True, bw=.4, inner=None, palette=palette,
                        linewidth=.5, ax=ax)
    # ax.set_aspect(1)
    ...
    fontProperties = {'family': 'Arial','weight': 'normal', 'size': 12}
    ...
    ax.tick_params(axis="y", direction="out", pad=200)
